...
public class CyclomaticComplexity {
	public static 	final	ArrayList<String> methodNameAll 	  = new ArrayList<String>();//存放package內所有檔案方法之名稱
	...
	public static   final   ArrayList<Double>countSubMethodListAll= new ArrayList<Double>(); //存放package內所有檔案每方法子方法數
         ...
         //輸出整個package中各檔案之資訊
          int count=0;
         	while(count<partmethod){    
         		...
         		opt.addData(methodNameAll.get(tempcountmethod+count),classNameAll.get(tempcountmethod+count),methodListAll.get(tempcountmethod+count),(methodListAll.get(tempcountmethod+count)>10)?"高":"低", countSubMethodListAll.get(tempcountmethod+count),subHighLowListAll.get(tempcountmethod+count), methodObjAll.get(tempcountmethod+count),objHighLowListAll.get(tempcountmethod+count));
